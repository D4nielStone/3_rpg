function colliderBounds(entity) {
  ...
  const halfX = Math.max(0.25, Math.abs(Number(scale[0]) || 1) * Math.abs(Number(collisionScale[0]) || 1) * 0.5);
  const halfZ = Math.max(0.25, Math.abs(Number(scale[2]) || 1) * Math.abs(Number(collisionScale[2]) || 1) * 0.5);
